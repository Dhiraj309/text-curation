...
This project exists to provide deterministic, structure-aware, production-grade text curation for large-scale NLP datasets.  
The goal is not clever cleaning — it is reproducible, auditable data preprocessing.
...
- Reporting schemas are **total**: fields are never conditionally omitted
...
- **never included in stable profiles unless explicitly promoted in a major release**

---

## 🔐 Profile & Registry Invariants

- Profile IDs are globally unique and immutable
- Duplicate profile registration is a fatal error
- Profiles are treated as behavioral contracts
- Profile identity fully determines behavior

These invariants are enforced by tests and must not be weakened.
...
Profiles must be explicitly versioned (e.g. web_common_v1) to ensure long-term reproducibility.
...
Profiles are versioned independently (e.g. web_common_v1) to preserve reproducibility across releases.

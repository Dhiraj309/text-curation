    ...
    def _normalize_paragraph_boundaries(self, text):
        ...
        for line in lines:
            ...
            if buffer:
                ...
                # Only merge wrapped prose:
                # 1) previous line not sentence-ending
                # 2) current line starts lowercase
                if (
                    _SENTENCE_END.search(prev)
                    or not _CONTINUATION_START.match(stripped)
                ):
                    out.append(" ".join(buffer))
                    buffer = []
